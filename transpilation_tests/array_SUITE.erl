...
%% TODO: Fix imports in erlps :(
new() -> array:new().
new(X) -> array:new(X).
new(X,Y) -> array:new(X,Y).
is_array(X) -> array:is_array(X).
set(X,Y,Z) -> array:set(X,Y,Z).
get(X,Y) -> array:get(X,Y).
sparse_size(X) -> array:sparse_size(X).
default(X) -> array:default(X).
reset(X,Y) -> array:reset(X,Y).
to_list(X) -> array:to_list(X).
sparse_to_list(X) -> array:sparse_to_list(X).
from_list(X) -> array:from_list(X).
from_list(X,Y) -> array:from_list(X,Y).
to_orddict(X) -> array:to_orddict(X).
sparse_to_orddict(X) -> array:sparse_to_orddict(X).
from_orddict(X) -> array:from_orddict(X).
from_orddict(X,Y) -> array:from_orddict(X,Y).
map(X,Y) -> array:map(X,Y).
sparse_map(X,Y) -> array:sparse_map(X,Y).
foldl(X,Y,Z) -> array:foldl(X,Y,Z).
foldr(X,Y,Z) -> array:foldr(X,Y,Z).
sparse_foldl(X,Y,Z) -> array:sparse_foldl(X,Y,Z).
sparse_foldr(X,Y,Z) -> array:sparse_foldr(X,Y,Z).
fix(X) -> array:fix(X).
relax(X) -> array:relax(X).
is_fix(X) -> array:is_fix(X).
resize(X) -> array:resize(X).
resize(X,Y) -> array:resize(X,Y).
